...
### Stage 4: データ品質改善と高度分析（新規追加・未開始）

#### P0: Critical - データ品質改善（最優先実装）

##### 4-1. トピック類似度クラスタリング
**目的:** 類似トピックを統合し、真の共通テーマを発見
**実装:**
- OpenAI Embeddings でトピック名をベクトル化
- Cosine類似度計算（閾値: 0.85以上）
- 類似トピック自動グルーピング
- 「キャリア」「AI技術」「起業」などの大カテゴリ抽出

**期待効果:**
- 23トピック → 5-8カテゴリに集約
- 共通トピック発見率向上（0個 → 5-10個）
- 時系列分析の精度向上

**コスト:** $0.0001-0.0005（23トピック分）

##### 4-2. エンティティ名寄せ
**目的:** 同一エンティティの異なる表記を統合
**実装:**
- RapidFuzz による文字列類似度計算
- 閾値ベースマージ（人物: 90%、組織: 85%）
- 手動確認オプション（境界ケース）

**期待効果:**
- 64名 → 50-55名に正規化
- 52組織 → 40-45組織に正規化
- 繰り返し登場エンティティ増加（1名 → 5-10名）
...
**実装:**
- Gemini API で構造化抽出
- JSON Schema: {action, assignee, deadline, priority, status}
- 未定義項目は「TBD」で明示
...
#### Stage 4（未開始）
- OpenAI Embeddings (トピッククラスタリング)
- RapidFuzz (エンティティ名寄せ)
- WhisperX (話者識別)
- transformers/TextBlob (センチメント分析)
- scikit-learn (Cosine類似度)
